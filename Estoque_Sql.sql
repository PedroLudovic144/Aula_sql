...
SELECT 
    p.codproduto,
    p.descricao,
    (SUM(ie.qtde) - SUM(isd.qtde)) AS estoque_atual
FROM 
    Produto p, ItemEntrada ie, ItemSaida isd
WHERE 
    p.codproduto = ie.Produto_codproduto 
    AND p.codproduto = isd.Produto_codproduto
GROUP BY 
    p.codproduto, p.descricao
HAVING 
    (SUM(ie.qtde) - SUM(isd.qtde)) > 0;
...
WHERE 
    ...
    AND p.codproduto = isd.Produto_codproduto
GROUP BY 
    p.codproduto, p.descricao, p.qtdemin
HAVING 
    (SUM(ie.qtde) - SUM(isd.qtde)) < p.qtdemin;
...
WHERE 
    p.Fornecedor_codfornecedor = f.codfornecedor
ORDER BY 
    f.fornecedor, p.codproduto;
...
Listagem 6. tempo médio de chegada do pedido.
    ...
    e.dataped,
    e.dataentr,
    DATEDIFF(e.dataentr, e.dataped) AS tempo_chegada
FROM 
    Entrada e;
...
Listagem 7. Total de saída separados por loja.
	SELECT 
    l.nome AS loja,
    SUM(s.total) AS total_saida
FROM 
    Saida s, Loja l
WHERE 
    s.Loja_codloja = l.codloja
GROUP BY 
    l.nome;
...
Listagem 10. Quantidade de item por categoria.
	...
	SELECT 
    c.categoria,
    (SUM(ie.qtde) - SUM(isd.qtde)) AS quantidade_total
FROM 
    Categoria c, Produto p, ItemEntrada ie, ItemSaida isd
WHERE 
    p.Categoria_codcategoria = c.codcategoria
    AND p.codproduto = ie.Produto_codproduto
    AND p.codproduto = isd.Produto_codproduto
GROUP BY 
    c.categoria;
